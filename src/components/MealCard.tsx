...
import { Users, CheckCircle2, Clock, Smartphone, QrCode, X } from 'lucide-react'
...
export default function MealCard({ meal, members, onParticipantChanged }: Props) {
    ...
    <>
      <div className={`rounded-2xl overflow-hidden transition-all duration-200 hover:-translate-y-0.5 ${
        ...
        <div className={`transition-colors duration-500 ${
          ...
          {hasTransferInfo && (
            <div className="mx-4 mb-3 mt-1 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-700 px-3 py-2.5">
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2">
                Chuyển khoản cho {meal.payer_name}
              </p>
              <div className="flex items-center flex-wrap gap-2">
                {payer?.momo_phone && (
                  <span className="inline-flex items-center gap-1.5 text-xs bg-pink-50 dark:bg-pink-900/20 text-pink-700 dark:text-pink-300 border border-pink-200 dark:border-pink-800 px-2.5 py-1 rounded-full font-semibold">
                    <Smartphone className="w-3 h-3" />
                    {payer.momo_phone}
                    <span className="text-pink-400 dark:text-pink-600 font-normal">MoMo</span>
                  </span>
                )}
                {payer?.qr_image_url && (
                  <button
